...
class App extends Component {
  ...
  handleCheckUserIncludeImage = imageid => {
    const { userimageinfo } = this.props.all_userimage

    if (this.props.all_userimage.userimageinfo.length > 0) {
      const userimgdata = userimageinfo.filter(userimg => {
        return userimg.imageid == imageid
      })
  ...
  handleSetUserImage = () => {
    const imglist = this.props.imagelist.map((img, idx) => {
      let userimgflag = this.handleCheckUserIncludeImage(img.id)
      return { ...img, userimgflag }
    })
    return imglist
  }
    ...
    return (
      <div className="app-wrapper">
        {imagelist ? (
          status.userID ? (
            <ImageList
              imagelist={this.handleSetUserImage()}
...
const mapDispatchToProps = dispatch => ({
  ...
  imageRequest: (searchname, pagenum, search_tag) =>
    dispatch(images.imageRequest(searchname, pagenum, search_tag)),
  set_searchname: searchname => dispatch(images.set_searchname(searchname)),
  userImageallRequest: userid =>
    dispatch(userimage.userImageallRequest(userid)),
})
